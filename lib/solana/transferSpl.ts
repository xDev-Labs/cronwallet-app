...
export const transferSpl = async (amount: number, smartAccountAddress: string, to: string, splTokenAddress: string, ownerPublicKey: PublicKey) => {
    console.log('🚀 Starting SPL transfer...');
    console.log('Amount:', amount);
    console.log('Smart Account:', smartAccountAddress);
    console.log('Recipient:', to);
    console.log('Token:', splTokenAddress);
    console.log('Owner:', ownerPublicKey.toBase58());

    let mintAddress = new PublicKey(splTokenAddress);
    ...
    // Get the destination token account (recipient's ATA)
    const dstTokenAccount = getAssociatedTokenAddressSync(
        mintAddress,
        recipientPublicKey,
        true,
        TOKEN_PROGRAM_ID,
    ...
    let transferTransactionIx = new TransactionInstruction({
        ...
        keys: [
            { pubkey: smartAccountPublicKey, isSigner: false, isWritable: false }, // smart_account
            { pubkey: ownerPublicKey, isSigner: true, isWritable: false }, // owner (must sign)
            { pubkey: srcTokenAccount, isSigner: false, isWritable: true }, // src_token_account
            { pubkey: dstTokenAccount, isSigner: false, isWritable: true }, // dst_token_account
